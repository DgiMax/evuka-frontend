...
const FilterSection = ({ title, children, defaultOpen = true }: any) => {
  ...
  return (
    <div className="border-b border-gray-100 last:border-0">
      <button 
        className="w-full flex justify-between items-center py-4 px-1 text-left group" 
        onClick={() => setIsOpen(!isOpen)}
      >
        <h3 className="font-black text-[11px] uppercase tracking-widest text-gray-900 group-hover:text-[#2694C6] transition-colors">
          {title}
        </h3>
        <ChevronUp className={cn("w-4 h-4 text-gray-400 transition-transform duration-300", !isOpen && "rotate-180")} />
      </button>
      {isOpen && (
        <div className="overflow-hidden pb-5 px-1 animate-in slide-in-from-top-1 duration-300 flex flex-col gap-1">
          {children}
        </div>
      )}
    </div>
...
export default function EventFilters({ data, onFilterChange }: any) {
  ...
  const clearAll = () => {
    const reset = { type: {}, category: {}, price: "all", upcoming: "all" };
    setFilters(reset);
    onFilterChange?.({});
  };
  ...
  const notify = (state: any) => {
    const params: any = {};
    const types = Object.keys(state.type).filter(k => state.type[k]);
            ...
            {data.upcoming_options?.map((opt: any) => (
              <label key={opt.id} className="flex items-center py-2 cursor-pointer group">
                <input 
                  type="radio" 
                  name="upcoming" 
                  checked={filters.upcoming === opt.id} 
                  onChange={() => handleRadio("upcoming", opt.id)} 
                  className="h-4 w-4 border-2 border-gray-300 transition-all accent-[#2694C6]" 
                />
                <span className={cn(
                  "ml-3 text-[13px] transition-colors", 
                  filters.upcoming === opt.id ? "text-[#2694C6] font-bold" : "text-gray-600 group-hover:text-gray-900"
                )}>
                  {opt.label}
                </span>
              </label>
            ...
            {data.categories?.map((cat: any) => (
              <label key={cat.id} className="flex items-center py-2 cursor-pointer group">
                <input 
                  ...
                  onChange={(e) => handleToggle("category", cat.id, e.target.checked)} 
                ...
                <span className={cn(
                  "ml-3 text-[13px] transition-colors", 
                  filters.category[cat.id] ? "text-[#2694C6] font-bold" : "text-gray-600 group-hover:text-gray-900"
                )}>
                  {cat.label}
                </span>
              </label>
            ...
            {data.price_options?.map((opt: any) => (
              <label key={opt.id} className="flex items-center py-2 cursor-pointer group">
                <input 
                  ...
                  onChange={() => handleRadio("price", opt.id)} 
                  className="h-4 w-4 border-2 border-gray-300 transition-all accent-[#2694C6]" 
                />
                <span className={cn(
                  "ml-3 text-[13px] transition-colors", 
                  filters.price === opt.id ? "text-[#2694C6] font-bold" : "text-gray-600 group-hover:text-gray-900"
                )}>
                  {opt.label}
                </span>
              </label>
