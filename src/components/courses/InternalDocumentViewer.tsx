...
const InternalDocumentViewer = ({ resource, onClose }: { resource: Resource, onClose: () => void }) => {
    ...
    return (
        <div className="w-full bg-[#1a1a1a] rounded-md border border-gray-800 overflow-hidden flex flex-col h-[85vh] animate-in slide-in-from-bottom-4 duration-500 shadow-2xl">
                ...
                <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-[9px] text-gray-400 font-bold uppercase tracking-tighter">
                            {format || 'Unknown'} {isBook ? "Edition" : "Mode"}
                        </span>
                    </div>
                </div>
                    ...
                    {absoluteUrl && isPdf ? (
                        ...
                        >
                            <img 
                                src={absoluteUrl} 
                                alt="Preview" 
                                className="max-w-full max-h-[70vh] shadow-2xl rounded border border-white/10" 
                            />
                        </div>
                    ) : absoluteUrl && isAudio ? (
                        <div className="flex flex-col items-center gap-8 p-12 text-center">
                            <div className="w-56 h-56 rounded-xl shadow-2xl overflow-hidden border-4 border-white/10 bg-[#1a1a1a]">
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center p-10">
                            ...
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-2 mb-8 max-w-xs">
                                {absoluteUrl 
                                    ? `This ${format || 'unsupported'} asset cannot be previewed in the secure reader.`
                                    : "This resource does not have a valid file attached."
                                }
                            </p>
                            {absoluteUrl && !isBook ? (
                                <a href={absoluteUrl} download className="bg-[#2694C6] text-white px-10 py-4 rounded-md text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#1e7ca8] transition-all shadow-lg flex items-center gap-2">
                                    <Download size={14} /> Download Asset
                                </a>
                            ) : isBook && absoluteUrl ? (
                                <p className="text-[9px] text-orange-500 font-black uppercase tracking-widest bg-orange-500/10 px-4 py-2 rounded">Encrypted Platform Content</p>
                            ) : null}
                        </div>
                    )}
                </div>
...
export default InternalDocumentViewer;