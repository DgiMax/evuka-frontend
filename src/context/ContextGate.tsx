    ...
    const verify = async () => {
      ...
      try {
        ...
        if (data.org_status === "draft") {
          toast.error("This organization is currently in draft mode.", { id: "draft-lock" });
          setActiveSlug(null);
          setActiveRole(null);
        ...
        if (data.is_active === false) {
          toast.error("Your membership is currently inactive.", { id: "inactive-member" });
        ...
        if (errorType === "no_membership") {
          toast.info("Join this organization to access its contents.");
          router.replace(`/organizations/browse/${slug}`);
        } else if (errorType === "draft_mode") {
          toast.error("Access denied: Organization is in draft mode.");
          router.replace("/dashboard");
        } else {
          router.replace("/dashboard");
        }
